...
func newSSHKeypairTestSecret(t *testing.T, extraAnnotations map[string]string, initialized bool) *corev1.Secret {
	...
	if initialized {
		keypair, err := secret.GenerateSSHKeypair(secret.SSHKeyLength())
		if err != nil {
			t.Error(err, "could not generate new ssh keypair")
		}
		s.Data[secret.SecretFieldPublicKey] = keypair.PublicKey
		s.Data[secret.SecretFieldPrivateKey] = keypair.PrivateKey
...
func verifySSHKeypairSecret(t *testing.T, in, out *corev1.Secret) {
	if out.Annotations[secret.AnnotationSecretType] != string(secret.SecretTypeSSHKeypair) {
