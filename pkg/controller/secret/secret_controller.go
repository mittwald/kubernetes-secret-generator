...
func (r *ReconcileSecret) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	toGenerate, ok := instance.Annotations[SecretGenerateAnnotation]
	if !ok {
		return reconcile.Result{}, nil
	}
	...
	if _, ok := instance.Annotations[SecretSecureAnnotation]; !ok && regenerateInsecure() {
		reqLogger.Info("instance was generated by cryptographically insecure PNRG")
		regenKeys = genKeys // regenerate all keys
	} else {
		if regenerate, ok := instance.Annotations[SecretRegenerateAnnotation]; ok {
			reqLogger.Info("removing regenerate annotation from instance")
			delete(instance.Annotations, SecretRegenerateAnnotation)
	...
	if generatedCount == len(genKeys) {
		...
		instance.Annotations[SecretSecureAnnotation] = "yes"
	}

	instance.Annotations[SecretGeneratedAtAnnotation] = time.Now().String()
